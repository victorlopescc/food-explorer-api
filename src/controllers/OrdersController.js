...
class OrdersController {
    async create(request, response) {
        ...
        const items = order_items.map(async ({ dish_id, quantity }) => {
        ...
        await knex('order_items').insert(await Promise.all(items));
    ...
    async show(request, response) {
        ...
        const items = await knex('order_items').where({ order_id: id }).orderBy('name');
    ...
    async index(request, response) {
        ...
        if (user.is_admin) {
            orders = await knex('orders').select([
                ...
                'orders.created_at'
            ]).innerJoin('users', 'users.id', 'orders.created_by').orderBy('orders.created_at', 'desc');
        } else {
            orders = await knex('orders').select([
                'orders.id',
                'orders.status',
                ...
                'orders.payment_method',
                'orders.created_at'
            ]).where({ created_by: user_id }).orderBy('orders.created_at', 'desc');
        }
        ...
        const ordersWithDishes = orders.map(order => {
            ...
            const filtered = user.is_admin ? orderDishes : orderDishes.map(({ name, quantity }) => ({ name, quantity }));
